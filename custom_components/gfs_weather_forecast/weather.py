...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            GfsForecastWeather(
                coordinator=hass.data[DOMAIN][entry.entry_id], entry_id=entry.entry_id
            )
        ]
    )
...
class GfsForecastWeather(WeatherEntity):
    ...
    def __init__(
        self, coordinator: GfsForecastDataUpdateCoordinator, entry_id: str
    ) -> None:
    ...
    def _get_forecast(self) -> list[Forecast] | None:
        ...
        for key in forecast_data.keys():
            ...
            if forecast_date > datetime.today().date():
                ...
                min_temperature_daytime = forecast_data[key].get(
                ...
                if temperature_max > -999 and temperature_min > -999:
                    next_day: Forecast = {
                        ...
                        ATTR_FORECAST_CONDITION: self._get_condition(
                            chance_of_sun, rain, min_temperature_daytime
                        ),
    ...
    def _get_condition(
        self, chance_of_sun: int, rain: float, temperature_min: float
    ) -> str:
        if rain > 0.2:
